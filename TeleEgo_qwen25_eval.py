...
SAVE_PRED_PATH: str = f"{BASE_DIR}/outputs/qwen25_vl/p{P_NUM}/eval_predictions_P{P_NUM}.json"
...
def parse_prediction(text: str, qtype: str, options: List[str]):
    """更严格的输出解析：只返回允许的字母；binary 返回 True/False/None；开放题返回原文。"""
    t = (text or "").strip()
...
def is_invalid_output(qtype: str, raw_text: str, options: List[str]) -> bool:
    """决定是否触发 retry：单选必须恰好 1 个允许字母；多选需至少 1 个允许字母；判断需 True/False。"""
    q = (qtype or "").lower()
    if q in {"mc_single", "mc_multi"}:
        ...
        if t == "":
            return True
        if q == "mc_single":
            # 只接受“一个字母”的纯输出且在允许集合内
            m = re.fullmatch(r"\s*([A-Za-z])\s*", t)
            if not m:
                # 退一步：从文本里抽第一个允许字母，没有也视为无效
...
# =============== Qwen2.5-VL init（trust_remote_code + right padding + im_end/eod） ===============
...
# =============== 生成（流式 + GenerationConfig + retry） ===============
BASE_SYS_TEXT = "你是一个多模态助手。根据当前秒的视频画面与1秒音频转写，严格按题目要求简洁作答。"

def encode_messages(processor, messages, image, use_utils: bool) -> Dict[str, torch.Tensor]:
...
def run_streaming_round(
    ...
    *,
    ...
    round_id: int = 0,
    prefer_simple_vision: bool = PREFER_SIMPLE_VISION,
    ...
    inputs = encode_messages(
        ...
        use_utils=(_HAS_QWEN_VL_UTILS and process_vision_info is not None and not prefer_simple_vision)
    )
